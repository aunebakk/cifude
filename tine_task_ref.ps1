...
param (
    ...
    [System.String]$tineTaskRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
...
try {
    if ($help) {
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            $taskLine = ([System.DateTime]::UtcNow.ToString() + ' ' + "[System.String] tineTaskRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')")
            $htmlLog = $htmlLog + $taskLine + '<br>'
            if ($doEcho) { Write-Host $taskLine }
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            if ( $tableTineTaskRef -eq $null -and !$whatIf ) {
                ...
                if (($tineTaskRef | Measure-Object).Count -gt 0) {
                    ...
                    $tineTaskRefData.tineTaskRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
