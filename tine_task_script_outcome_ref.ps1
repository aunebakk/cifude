...
# Generated Date: 7/29/2018 9:45:07 AM
...
param (
    ...
    [System.String]$tineTaskScriptOutcomeRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
...
try {
    if ($help) {
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            $taskLine = ([System.DateTime]::UtcNow.ToString() + ' ' + "[System.String] tineTaskScriptOutcomeRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')")
            $htmlLog = $htmlLog + $taskLine + '<br>'
            if ($doEcho) { Write-Host $taskLine }
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            if ( $tableTineTaskScriptOutcomeRef -eq $null -and !$whatIf ) {
                ...
                if (($tineTaskScriptOutcomeRef | Measure-Object).Count -gt 0) {
                    ...
                    $tineTaskScriptOutcomeRefData.tineTaskScriptOutcomeRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
