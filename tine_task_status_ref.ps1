...
param (
    ...
    [System.String]$tineTaskStatusRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
...
try {
    if ($help) {
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            $taskLine = ([System.DateTime]::UtcNow.ToString() + ' ' + "[System.String] tineTaskStatusRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')")
            $htmlLog = $htmlLog + $taskLine + '<br>'
            if ($doEcho) { Write-Host $taskLine }
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            if ( $tableTineTaskStatusRef -eq $null -and !$whatIf ) {
                ...
                if (($tineTaskStatusRef | Measure-Object).Count -gt 0) {
                    ...
                    $tineTaskStatusRefData.tineTaskStatusRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
