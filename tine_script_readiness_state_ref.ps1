...
param (
    ...
    [System.String]$tineScriptReadinessStateRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
...
try {
    if ($help) {
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            $taskLine = ([System.DateTime]::UtcNow.ToString() + ' ' + "[System.String] tineScriptReadinessStateRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')")
            $htmlLog = $htmlLog + $taskLine + '<br>'
            if ($doEcho) { Write-Host $taskLine }
        ...
        if ($answer -ne 'no' -and ($doDevelopment -or $doTest -or $doSql2x)) {
            ...
            if ( $tableTineScriptReadinessStateRef -eq $null -and !$whatIf ) {
                ...
                if (($tineScriptReadinessStateRef | Measure-Object).Count -gt 0) {
                    ...
                    $tineScriptReadinessStateRefData.tineScriptReadinessStateRefRowKey = [System.DateTime]::UtcNow.ToString('yyyy.MM.dd HH:mm:ss fff')
